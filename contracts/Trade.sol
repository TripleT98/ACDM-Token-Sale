...
contract Trade is ReentrancyGuard, DAOable {
  ...
  function setFee(uint _fee) public OnlyDAO {
    require(_fee < 500, "Error: Fee is too big!");
    fee = _fee;
  }

  function setRef1Fee(uint _fee) public OnlyDAO {
    require(_fee + ref2Fee < 100, "Error: Referal `1` fee is too big!");
    ref1Fee = _fee;
  }

  function setRef2Fee(uint _fee) public OnlyDAO {
    require(_fee + ref1Fee < 100, "Error: Referal `2` fee is too big!");
    ref2Fee = _fee;
  }
  ...
  function setACDM(address _acdm) public {
    require(msg.sender == admin, "Error: Only admin has access!");
    ACDMToken = _acdm;
  }
  ...
  function _setTradeRound() internal {
    require(saleRound.endTime <= block.timestamp, "Error: It is not time yet for trade!");
  ...
  function register(address _referal) public {
    ...
    require(msg.sender != _referal, "Error: Referal`s address equals to yours!");
  ...
  function redeemOrder(uint _amount, uint _id) OnlyTradeRound payable public {
    ...
    require(currentOrder.owner != msg.sender, "Error: Can`t redeem your oreder!");
